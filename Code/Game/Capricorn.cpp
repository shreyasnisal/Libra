...
Capricorn::Capricorn(EntityType type, EntityFaction faction, Vec2 const& startPosition, float orientationDegrees)
...
void Capricorn::InitializeVertexes()
{
...
void Capricorn::SetGoalPosition()
{
...
void Capricorn::PickRandomGoalPosition()
{
	if (!m_map)
	{
		return;
...
void Capricorn::GenerateHeatMap()
{
	if (!m_map)
	{
		return;
...
void Capricorn::SetNextWaypointPosition()
{
...
bool Capricorn::IsPointReachable()
{
...
void Capricorn::TurnAndMoveTowardsNextWaypointPosition(float deltaSeconds)
{
...
void Capricorn::SetPlayerAsGoalPositionIfPlayerIsVisible()
{
	Entity* player = m_map->m_entitiesByType[ENTITY_TYPE_GOOD_PLAYER][0];
...
void Capricorn::CheckPlayerPositionAndFire(float deltaSeconds)
{
	Entity* player = m_map->m_entitiesByType[ENTITY_TYPE_GOOD_PLAYER][0];
...
bool Capricorn::CanTakeShortcut()
{
...
void Capricorn::TurnAndMoveTowardsGoalPosition(float deltaSeconds)
{
...
void Capricorn::Render() const
{
	...
	g_renderer->BindTexture(g_textures[CAPRICORN_BASE_TEXTURE]);
...
void Capricorn::FireBullet()
{
	...
	m_map->SpawnNewEntityOfType(ENTITY_TYPE_EVIL_BULLET, m_position + m_cosmeticRadius * GetForwardNormal(), m_orientationDegrees);
}
